...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Arkain Atlas API [env={settings.env}]")
    ...
    # Shutdown
    logger.info("Shutting down Arkain Atlas API")
...
# ═══════════════════════════════════════════════════════════════
# STREAMING CHAT ENDPOINT (frontend expects /stream)
# ═══════════════════════════════════════════════════════════════
...
async def stream_chat(body: StreamBody):
    """NDJSON streaming endpoint for chat interface.

    The frontend sends a message, we run the v3 engine and stream
    events back as NDJSON (newline-delimited JSON).

    Event types: message_delta, tool_call, tool_result, run_event,
                 artifact_created, final_message, error
    """
    async def generate():
        try:
            # Phase 1: acknowledge
            yield _json.dumps({"type": "message_delta", "content": "Analyzing your request... "}) + "\n"
            await asyncio.sleep(0.05)

            # Run v3 engine if it looks like a deal
            content_lower = body.content.lower()
            is_deal_query = any(w in content_lower for w in [
                "deal", "property", "investment", "analyze", "buy",
                "noi", "cap rate", "irr", "gaming", "gas station",
                "acquisition", "evaluate", "due diligence",
            ])

            if is_deal_query:
                yield _json.dumps({"type": "run_event", "content": "Initializing Truth Accretion Engine..."}) + "\n"
                await asyncio.sleep(0.05)
                ...
                seed_engine(run_id=f"stream_{body.threadId}")
                registry = create_tool_registry(live=True)

                yield _json.dumps({"type": "tool_call", "name": "tool_registry", "input": {"tools": len(registry.available_tools())}}) + "\n"
                await asyncio.sleep(0.05)

                orch = TruthAccretionOrchestrator(
                    execute_fn=lambda **kw: {"recommendation": "NEEDS_DATA", "analysis": body.content},
                    tool_registry=registry,
                )

                # Parse any deal data from the message
                deal_data = {"address": "User Query", "property_type": "gas_station", "description": body.content}
                agents = ["underwriting_analyst", "risk_officer", "market_analyst"]
                agent_tools = {
                    "underwriting_analyst": ["census_demographics", "pull_comps", "fred_economic_data"],
                    "risk_officer": ["crime_data", "environmental_risk"],
                    "market_analyst": ["traffic_counts", "competitor_scan"],
                }

                pkg = orch.run(deal_data=deal_data, active_agents=agents, agent_tools=agent_tools)

                for tool_call in (pkg.tool_calls or [])[:5]:
                    yield _json.dumps({
                        "type": "tool_result",
                ...
                # Stream the decision
                analysis_text = (
                    f"**Decision: {pkg.decision}** (Confidence: {pkg.confidence:.0%})\n\n"
                    f"The analysis ran {pkg.total_tool_calls} tool calls across {len(agents)} agents. "
                    f"Grounding score: {pkg.grounding_pct:.1f}% of variables backed by evidence.\n\n"
                )
                if pkg.decision == "NEEDS_DATA":
                    missing = pkg.missing_variables[:5] if pkg.missing_variables else []
                    if missing:
                        analysis_text += "**Key data still needed:**\n"
                        for v in missing:
                            analysis_text += f"- {v}\n"

                for chunk in [analysis_text[i:i+40] for i in range(0, len(analysis_text), 40)]:
                    yield _json.dumps({"type": "message_delta", "content": chunk}) + "\n"
                ...
                yield _json.dumps({
                    ...
                    "artifact": {
                        ...
                        "data": {
                            ...
                            "tool_calls": pkg.total_tool_calls,
                            "seed": pkg.seed,
                            "ledger_verified": pkg.ledger_verified,
                        },
                    },
                }) + "\n"
                ...
                response = (
                    "I'm **Arkain Atlas**, your CRE deal intelligence engine. "
                    "I can help you with:\n\n"
                    "• **Deal analysis** — submit a property for full 17-agent evaluation\n"
                    "• **Monte Carlo simulation** — correlated risk modeling\n"
                    "• **Capital stack optimization** — senior/mezz/equity waterfall analysis\n"
                    "• **Scenario planning** — stress test across 5 macro futures\n"
                    "• **Market research** — demographics, traffic, comps, gaming data\n\n"
                    "Use the **New Deal** button to submit structured property data, "
                    "or ask me anything about CRE investment analysis."
                )
                for chunk in [response[i:i+50] for i in range(0, len(response), 50)]:
                    yield _json.dumps({"type": "message_delta", "content": chunk}) + "\n"
                    await asyncio.sleep(0.02)

            yield _json.dumps({"type": "final_message"}) + "\n"
...
# ═══════════════════════════════════════════════════════════════
# THREAD MANAGEMENT (frontend expects /threads)
# ═══════════════════════════════════════════════════════════════
...
def list_threads():
    ...
    return sorted(_threads_store.values(), key=lambda t: t.get("updatedAt", ""), reverse=True)
...
def create_thread(body: dict):
    ...
    _thread_counter += 1
    tid = f"thread-{_thread_counter}"
    thread = {
        "id": tid,
        "title": body.get("title", f"New Analysis #{_thread_counter}"),
        "lastMessage": "",
        "updatedAt": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "unread": False,
    }
    _threads_store[tid] = thread
    return thread


# ═══════════════════════════════════════════════════════════════
# ARTIFACT MANAGEMENT (frontend expects /artifacts)
# ═══════════════════════════════════════════════════════════════
...
def list_artifacts(threadId: str = ""):
    """List artifacts, optionally filtered by thread."""
    arts = list(_artifacts_store.values())
    if threadId:
        arts = [a for a in arts if a.get("threadId") == threadId]
    return arts
...
def get_artifact(artifact_id: str):
    ...
    if artifact_id not in _artifacts_store:
        raise HTTPException(404, "Artifact not found")
    return _artifacts_store[artifact_id]


@app.post("/api/v1/artifacts/{artifact_id}/pin")
def pin_artifact(artifact_id: str):
    """Toggle pin on an artifact."""
    if artifact_id not in _artifacts_store:
        raise HTTPException(404, "Artifact not found")
    _artifacts_store[artifact_id]["pinned"] = not _artifacts_store[artifact_id].get("pinned", False)
    return _artifacts_store[artifact_id]


# ═══════════════════════════════════════════════════════════════
# STRATEGIC DISPATCHER (frontend calls /strategic?sub=analyze)
# ═══════════════════════════════════════════════════════════════

@app.post("/api/v1/strategic")
def strategic_dispatcher(
    request: Request,
    body: dict,
    session=Depends(get_db_session),
):
    """Dispatch strategic requests from frontend.

    Frontend calls: /strategic?sub=analyze
    Backend routes to the correct handler.
    """
    sub = request.query_params.get("sub", "analyze")
    auth_result = None
    try:
        auth_result = authenticate(request)
    except Exception:
        # Allow unauthenticated for now (dev mode)
        from engine.auth import AuthResult
        auth_result = AuthResult(
            user_id="dev-user",
            workspace_id="dev-workspace",
            email="dev@arkain-atlas.ai",
            role="admin",
        )

    if sub == "analyze":
        from engine.strategic.pipeline import StrategicPipeline
        pipeline = StrategicPipeline(session, auth_result.workspace_id, auth_result.user_id)
        return pipeline.analyze(body)
    elif sub == "swot":
        from engine.strategic.pipeline import StrategicPipeline
        pipeline = StrategicPipeline(session, auth_result.workspace_id, auth_result.user_id)
        return pipeline.swot_only(body)
    elif sub == "scenarios":
        from engine.strategic.pipeline import StrategicPipeline
        pipeline = StrategicPipeline(session, auth_result.workspace_id, auth_result.user_id)
        return pipeline.scenario_analysis(body)
    elif sub == "stress-test":
        from engine.strategic.pipeline import StrategicPipeline
        pipeline = StrategicPipeline(session, auth_result.workspace_id, auth_result.user_id)
        return pipeline.stress_test(body)
    else:
        raise HTTPException(400, f"Unknown sub-action: {sub}")


# ═══════════════════════════════════════════════════════════════
# V3 ENGINE ENDPOINTS (capital stack, scenarios, MC)
# ═══════════════════════════════════════════════════════════════

@app.post("/api/v3/deals/monte-carlo")
def v3_monte_carlo(body: dict):
    ...
    mc = MonteCarloSimulator(body.get("num_simulations", 1000))
    return mc.simulate_deal(body)


@app.post("/api/v3/deals/capital-stack/waterfall")
def v3_capital_waterfall(body: dict):
    """Compute capital stack waterfall."""
    from engine.brain.capital_stack import (
        CapitalStack, DebtTranche, EquityTranche, compute_waterfall,
    )
...
@app.post("/api/v3/deals/capital-stack/stress-test")
def v3_capital_stress(body: dict):
    """Stress test capital structure with correlated MC."""
    from engine.brain.capital_stack import (
        CapitalStack, DebtTranche, EquityTranche, stress_test_stack,
    )
    from engine.brain.determinism import seed_engine
...
@app.post("/api/v3/deals/scenarios/evaluate")
def v3_scenario_evaluate(body: dict):
    """Evaluate CRE scenario tree with CRRA utility."""
    from engine.brain.scenario_tree import ScenarioTreeEvaluator, cre_standard_tree
    from engine.brain.determinism import seed_engine
    seed_engine(run_id=f"scenario_{int(time.time())}")
    base = body.get("base_params", body)
    tree = cre_standard_tree(base)
    ev = ScenarioTreeEvaluator(
        base, num_simulations=body.get("num_simulations", 500),
        risk_aversion=body.get("risk_aversion", 2.0),
    )
    return ev.evaluate(tree)


@app.post("/api/v3/deals/run")
def v3_full_pipeline(body: dict):
    """Run full v3 Truth Accretion pipeline."""
    from engine.brain.orchestrator_v3 import TruthAccretionOrchestrator
    from engine.brain.tools_unified import create_tool_registry
    from engine.brain.determinism import seed_engine

    seed_engine(run_id=body.get("run_id", f"deal_{int(time.time())}"))
    registry = create_tool_registry(live=True)

    orch = TruthAccretionOrchestrator(
        execute_fn=lambda **kw: {"recommendation": "NEEDS_DATA"},
        tool_registry=registry,
    )

    pkg = orch.run(
        deal_data=body.get("deal_data", body),
        active_agents=body.get("agents", ["underwriting_analyst", "risk_officer", "market_analyst"]),
    ...
    return {
        ...
        "confidence": pkg.confidence,
        "grounding_pct": pkg.grounding_pct,
        "total_tool_calls": pkg.total_tool_calls,
        "missing_variables": pkg.missing_variables,
        "seed": pkg.seed,
        "ledger_verified": pkg.ledger_verified,
    }


@app.get("/api/v3/engine/health")
def v3_engine_health():
    """V3 engine health check."""
    from engine.brain.tools_unified import create_tool_registry
    registry = create_tool_registry(live=True)
    return {
        "status": "ok",
        "version": "3.3.1",
        "tools_registered": len(registry.available_tools()),
        "modules": [
            "orchestrator_v3", "active_inference", "seal_ceca",
            "correlated_mc", "capital_stack", "scenario_tree",
            "cost_aware_forager", "ledger", "security",
        ],
    }
