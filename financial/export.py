...
def _csv_amortization(
    ...
    writer.writerow(["Generated by Arkain Atlas", datetime.now(timezone.utc).isoformat()])
    writer.writerow([])
...
def _pdf_reportlab(
    ...
    story.append(Paragraph(f"Generated by Arkain Atlas | {ts}", styles["Normal"]))
    if meta:
        if meta.get("run_id"):
...
def _pdf_fpdf2(
    ...
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    pdf.cell(0, 6, f"Generated by Arkain Atlas | {ts}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(5)
...
def _pdf_minimal(
    ...
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    lines.append(f"Generated by Arkain Atlas | {ts}")
    if meta:
        if meta.get("run_id"):
