...
=======================================
Canonical entrypoint for the Arkain Atlas.

Usage:
    arkain-atlas serve      # Start API server
    arkain-atlas migrate    # Run Alembic migrations
    arkain-atlas seed       # Seed default templates
    arkain-atlas check      # Health check
"""
...
def cmd_serve(args):
    ...
    print(f"Starting Arkain Atlas on {host}:{port} (env={settings.env})")
    uvicorn.run(
...
def cmd_check(args):
    ...
    try:
        with urllib.request.urlopen(url, timeout=5) as resp:
            ...
            print(f"✓ Arkain Atlas v{version} — {status}")
            sys.exit(0)
...
def main():
    parser = argparse.ArgumentParser(
        prog="arkain-atlas",
        description="Arkain Atlas CLI — enterprise agentic AI pipeline engine",
    )
